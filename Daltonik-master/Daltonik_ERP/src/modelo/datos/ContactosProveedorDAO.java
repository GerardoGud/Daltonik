...
public class ContactosProveedorDAO {
    ...
    public DefaultTableModel buscarId(JTable tDatos,int id) {
        ...
        tabla.setRowCount(0);
        try {
            r = cn.consultar("select cp.*, p.nombre nombre from ContactosProveedor cp join proveedores p on cp.idProveedor = p.idProveedor where cp.idContacto="+id+";");
            while (r.next()) {
                Vector dato = new Vector();
                dato.add(r.getInt(1));
                dato.add(r.getString(2));
    ...
    public DefaultTableModel buscarCadena(JTable tDatos,String cad) {
        ...
        tabla.setRowCount(0);
        try {
            r = cn.consultar("select cp.*, p.nombre nombre from ContactosProveedor cp join proveedores p on cp.idProveedor = p.idProveedor where cp.nombre like '%"+cad+"%';");
            while (r.next()) {
                Vector dato = new Vector();
                dato.add(r.getInt(1));
                dato.add(r.getString(2));
